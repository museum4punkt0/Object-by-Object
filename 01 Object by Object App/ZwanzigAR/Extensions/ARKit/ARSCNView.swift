...
extension ARSCNView {
	...
	func hitTestWithInfiniteHorizontalPlane(_ point: CGPoint, _ pointOnPlane: SCNVector3) -> SCNVector3? {
		...
		// at height (Y axis).
		return SCNVector3.rayIntersectionWithHorizontalPlane(rayOrigin: ray.origin, direction: ray.direction, planeY: pointOnPlane.y)
	}
	...
	func virtualObjectForHitTest(touch: UITouch, sessionObjects: [String: VirtualObject], combinationState: VirtualObject.CombinationState? = nil) -> (VirtualObject, SCNVector3)? {
		for result in hitTest(touch.location(in: self), options: [SCNHitTestOption.searchMode: SCNHitTestSearchMode.all.rawValue]) {
			if let virtualObject = VirtualObject.hitVirtualObject(node: result.node, virtualObjects: sessionObjects) {
				if combinationState == nil || virtualObject.combinationState == combinationState {
					return (virtualObject, result.localCoordinates)
				}
			}
		}
		return nil
